...
define Device/sun8i-v3s-dolphinpi
  ...
  DEVICE_DTS := sun8i-v3s-dolphinpi

  KERNEL := kernel-bin | lzma | uImage lzma 
  IMAGES := images.zip ubispinand.img.gz
  IMAGE/images.zip := sunxi-imageszip
  # IMAGE/spinand.img.gz := sunxi-spinand | pad-rootfs | append-metadata | gzip

  # One minimum I/O for VID, one for EC -> LEB = 124KiB
  UBIFS_OPTS := -F -m 2048 -e 124KiB -c 880 -U
  UBINIZE_OPTS := -vv
  ...
  KERNEL_IN_UBI := 1
  UBOOTENV_IN_UBI := 1
  UBINIZE_PARTS := dtb=$(DTS_DIR)/$$(DEVICE_DTS).dtb=1
  IMAGE/ubispinand.img.gz := \
      sunxi-ubispinand | \
      pad-to $$(CONFIG_SUNXI_SPINAND_OFFSET_DTB) | \
      append-ubi | \
      gzip
endef
